    ...
    <div className="flex flex-col items-center justify-center px-8 mx-auto lg:px-16 xl:px-0 py-20 z-1 max-w-6xl md:gap-8 gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8 text-gray-600 text-justify md:text-lg py-4 md:py-0">
        {LIVE_PROJECTS.map((proj) => (
          <LiveProjectCard project={proj} key={proj.name} isProjectsSection={true}/>
        ))}
        {projects.map((project) => (
          <Card
            key={project.title}
            className="p-4 md:p-6 lg:p-8 hover:scale-105 transition-transform duration-300 gap-4 md:gap-8"
          >
